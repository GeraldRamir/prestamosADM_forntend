...
const ClienteBody = ({ cliente, onEdit, onDelete }) => {
    const { clientesFiltrados, editarCliente } = useContext(ClientesContext);
    ...
    const handleFechaIngresoChange = async (clienteId, nuevaFecha) => {
        try {
            const clienteActual = clientesFiltrados.find(c => c._id === clienteId);
            const clienteActualizado = {
                ...clienteActual,
                fechaIngreso: nuevaFecha
            };
            await editarCliente(clienteActualizado); // Actualiza en la BD automáticamente
        } catch (error) {
            ...
            <tr>
                ...
                <td style={{ whiteSpace: "nowrap" }}>
                    <input
                        ...
                        value={cliente.FechaIngreso ? cliente.FechaIngreso.substring(0, 10) : ''}
                        onChange={(e) => handleFechaIngresoChange(cliente._id, e.target.value)}
                    />
                </td>
